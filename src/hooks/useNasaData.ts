...
function normalizeGeometry(raw: any[]): EventData['geometry'] {
  ...
  return raw
    .map((g) => {
      ...
      return {
        coordinates: pair,
        type: typeof g.type === 'string' ? g.type : 'Point',
...
export function useNasaEvents() {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    // Storms fizzle fast in EONET (marked closed within hours). Widen their window.
      ...
      categories.map(async (category) => {
        ...
        try {
          ...
          for (const e of data.events || []) {
            ...
            // Track-based events (storms) — use most recent point.
    ...
    const [quakes, emscQuakes, usgsVolcanoes] = await Promise.all([
      fetchEarthquakes(),
...
async function fetchEarthquakes(): Promise<EventData[]> {
  ...
  for (const url of urls) {
    try {
      ...
      console.warn(`USGS feed failed (${url}):`, error);
    }
  }
  return [];
}
      ...
      .map((f: any): EventData | null => {
        ...
        return {
          ...
          geometry: [{
            coordinates: pair,
            type: 'Point',
            date: new Date(time ?? Date.now()).toISOString(),
          }],
        };
      })
      .filter((e: EventData | null): e is EventData => e !== null);
    } catch (err) {
      console.warn(`EMSC feed failed (${url}):`, err);
      continue;
    }
  }
  return [];
...
async function fetchUsgsVolcanoes(): Promise<EventData[]> {
  for (const url of [USGS_VOLCANO_FEED, USGS_VOLCANO_FEED_DIRECT]) {
    try {
      ...
      const parsed = list
        .map((v: any): (EventData & { _lastEruption: number }) | null => {
      ...
      console.warn(`USGS volcano feed failed (${url}):`, err);
      continue;
    }
  }
  return [];
