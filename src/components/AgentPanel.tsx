...
import {
  Bot, ImagePlus, Camera, CircleDot, Send, X, Bookmark, Search,
...
function liveContext() {
  ...
  for (const [cat, catLabel] of Object.entries(label)) {
    ...
    const sample = list.slice(0, 4).map((e) => {
      const c = e.geometry?.[0]?.coordinates;
      const t = e.title.length > 70 ? e.title.slice(0, 70) + '…' : e.title;
      const mag = typeof e.magnitude === 'number' ? ` M${e.magnitude.toFixed(1)}` : '';
      return `  - ${t}${mag}${c ? ` @ ${c[1].toFixed(2)},${c[0].toFixed(2)}` : ''}`;
    });
    parts.push(`${catLabel} (${list.length} total):\n${sample.join('\n')}`);
  }
  const sel = selectedEvent
    ? `\nUser clicked marker: "${selectedEvent.title}" [${selectedEvent.category}]` +
      (selectedEvent.geometry?.[0]?.coordinates
        ? ` @ ${selectedEvent.geometry[0].coordinates[1].toFixed(2)},${selectedEvent.geometry[0].coordinates[0].toFixed(2)}`
        : '')
    : '';
...
export default function AgentPanel() {
  const {
    agentOpen, setAgentOpen, minimaxKey, nvidiaKey, aiProvider, aiModel,
    setShowApiKeyModal,
  } = useStore();
  ...
  useEffect(() => {
    listTurns(30).then(setTurns);
    listMemories(40).then(setMemories);
  }, []);
  ...
  const send = async (text: string, extraImages: string[] = []) => {
    ...
    const imgs = [...images, ...extraImages];
    if (!content && imgs.length === 0) return;
    if (!activeKey) {
      setError(`Add a ${provider.label} key in Providers first.`);
    ...
    // Include the last few turns so the conversation actually flows.
    ...
    if (!supportsVision && imgs.length > 0) {
      setError(`${provider.label} does not support image inputs. Sending text only.`);
    }
  ...
  const attachFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      next.push(await toPngDataUrl(file));
    }
    if (next.length) setImages((imgs) => [...imgs, ...next].slice(0, 4));
  };

  const capture = async () => {
    const shot = await requestGlobeScreenshot();
    if (shot) setImages((imgs) => [...imgs, shot].slice(0, 4));
    else setError('Could not capture the globe canvas.');
  };
  ...
  const record = async () => {
    ...
    try {
      ...
      if (!clip?.poster) {
        setError('Recording failed. Try again after the globe has rendered.');
        return;
      }
      setImages((imgs) => [...imgs, clip.poster].slice(0, 4));
      await saveMemory({
        kind: 'capture',
        title: 'Live globe clip',
        text: '4 second recording of the live globe (poster frame stored).',
        tags: ['capture', 'globe', 'live'],
        dataUrl: clip.poster,
        source: 'recorder',
      });
  ...
  const pinMemory = async () => {
    ...
    if (!last) return;
    const rec = await saveMemory({
      kind: 'note',
      title: last.content.slice(0, 72),
      text: last.content.slice(0, 2000),
      tags: ['pinned'],
      source: 'user',
    });
    setMemories((m) => [rec, ...m]);
    ...
    <aside className="agent-panel" aria-label="AI agent">
      <div className="side-header">
        <div className="side-kicker">
          <span className="side-kicker-icon"><Bot size={14} /></span>
          <h3>Researcher Agent</h3>
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          <button type="button" className="icon-btn" title="Pinned memory" onClick={pinMemory}>
            <Bookmark size={14} />
          </button>
          <button type="button" className="icon-btn" title="Memory" onClick={() => setShowMemory((v) => !v)}>
            <Search size={14} />
          </button>
          <button type="button" className="icon-btn" aria-label="Close agent" onClick={() => setAgentOpen(false)}>
            <X size={15} />
          </button>
        </div>
      </div>
          ...
          <div className="event-list" style={{ marginTop: 8, maxHeight: 160, overflow: 'auto' }}>
            ...
            {shownMemories.map((m) => (
              <button
                ...
                onClick={() => setDraft((d) => `${d} ${m.title}`.trim())}
              >
                <p className="event-title">{m.title}</p>
                <p className="event-meta">{m.kind} · {new Date(m.createdAt).toLocaleString()}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="agent-log custom-scrollbar" ref={scroller}>
        {turns.length === 0 && (
          <div className="side-empty">
            <Globe2 size={28} />
            <p>Ask about active earthquakes, volcanoes, storms, wildfires — or any question about natural disasters. Try:</p>
            <ul style={{ margin: '8px 0 0 0', padding: 0, listStyle: 'none', display: 'grid', gap: 4, fontSize: 11 }}>
              <li>"What storms are active right now?"</li>
              <li>"Explain the difference between a M6 and M7 quake."</li>
              <li>"Which volcanoes are erupting today?"</li>
              <li>"How do hurricanes form?"</li>
            </ul>
          </div>
        )}
        {turns.map((t) => (
          <div key={t.id} className={`agent-bubble ${t.role}`}>
            {t.images?.map((src) => (
              <img key={src.slice(-24)} src={src} alt="" className="agent-thumb" />
            ))}
            <p>{t.content}</p>
          </div>
        ))}
        {busy && (
          <div className="agent-bubble assistant">
            <Loader2 size={14} className="spin" /> Thinking with live search...
          </div>
        )}
      </div>
      ...
      {images.length > 0 && (
        <div className="agent-attach-row">
          {images.map((src) => (
            <img key={src.slice(-18)} src={src} alt="" className="agent-thumb" />
          ))}
        </div>
      )}
      ...
      >
        <div className="agent-tools">
          <button type="button" className="icon-btn" title="Attach image" onClick={() => fileRef.current?.click()}>
            <ImagePlus size={15} />
          </button>
          <button type="button" className="icon-btn" title="Screenshot globe" onClick={capture}>
            <Camera size={15} />
          </button>
          <button type="button" className="icon-btn" title="Record live globe" onClick={record} disabled={busy}>
            <CircleDot size={15} />
          </button>
        </div>
        <input
        ...
        />
        <textarea
          className="field-input agent-input"
          rows={2}
          placeholder="Ask the researcher... drop a map screenshot"
          value={draft}
        ...
        />
        <button type="submit" className="ui-btn primary" disabled={busy} aria-label="Send">
          <Send size={14} />
        </button>
      </form>
      <p className="layer-desc" style={{ padding: '0 12px 10px' }}>
        <Paperclip size={11} /> PNG, JPEG, WebP, GIF. Keys stay on this machine. Provider via /api/ai.
      </p>
    </aside>
  );
}
