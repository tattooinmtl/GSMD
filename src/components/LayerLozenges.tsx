...
import {
  Activity, Cloud, CloudLightning, CloudRain, Droplet, Droplets,
  Flame, Layers, Leaf, Lightbulb, Mountain, MountainSnow, Snowflake,
  Sun, Thermometer, TreePine, Waves, Wind, Anchor, TrendingUp
} from 'lucide-react';
...
export function LayerLozenge({
  ...
  return (
    <button
      type="button"
      className={`lozenge${layer.enabled ? ' is-on' : ''}${focused ? ' is-focus' : ''}`}
      style={{ '--glyph': layer.color } as CSSProperties}
...
export function LozengeRow({
  ...
  const [focusedId, setFocusedId] = useState<string | null>(
    layers.find((l) => l.enabled)?.id ?? layers[0]?.id ?? null
  );
    ...
    <div className="lozenge-block">
      <div className="lozenge-row">
        {layers.map((layer) => (
          <LayerLozenge
            key={layer.id}
            layer={layer}
            focused={showDetail && focused?.id === layer.id}
            onToggle={onToggle}
          />
        ))}
      </div>
      {showDetail && focused && (
        <div className="lozenge-detail">
          ...
          {focused.enabled && (
            <OpacitySlider
              value={focused.opacity}
              color={focused.color}
              onChange={(next) => setLayerOpacity(focused.id, next)}
            />
          )}
        </div>
      )}
