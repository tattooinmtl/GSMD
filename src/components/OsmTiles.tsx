...
function OsmTileMesh({ tile }: { tile: TileId }) {
  ...
  useEffect(() => {
    ...
    loadMapTexture(`/api/osm/${tile.z}/${tile.x}/${tile.y}.png`, key).then((tex) => {
      if (live) setMap(tex);
    });
    return () => {
      live = false;
  ...
  const geometry = useMemo(
    () => createSphereTileGeometry(tile, OSM_RADIUS),
    [tile.z, tile.x, tile.y]
  );
...
export default function OsmTiles() {
  ...
  return (
    <group>
      {tiles.map((t) => (
        <OsmTileMesh key={tileKey(t)} tile={t} />
      ))}
    </group>
  );
}
