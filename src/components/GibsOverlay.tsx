...
function GibsTileMesh({
  ...
  useEffect(() => {
    ...
    loadMapTexture(url, key).then((tex) => {
      if (live) setMap(tex);
    });
    return () => {
      live = false;
  ...
  const geometry = useMemo(
    () => createSphereTileGeometry({ z: tile.z, x: tile.x, y: tile.y }, radius),
...
export default function GibsOverlay() {
  ...
  const overlayIds = layers
    .filter((l) => l.enabled && GIBS_BY_LAYER[l.id])
    .map((l) => l.id);
  ...
  return (
    <group>
      {overlayIds.map((id, i) => (
        <GibsLayer key={id} layerId={id} radius={2.01 + i * 0.004} tiles={tiles} />
      ))}
    </group>
  );
}
