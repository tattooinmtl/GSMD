...
function dotTitle(status: StationStatus): string {
  switch (status) {
    case 'online':
      return 'Online — live MiniSEED data available';
    case 'offline':
      return 'Offline — no data in last 24 h';
    case 'partial':
      return 'Partial — station registered but no recent waveform data';
    case 'checking':
      return 'Checking…';
    default:
...
async function fetchMiniseed(
  activeId: DataCenterId,
  station: Station,
  cha: string,
  loc: string,
  windowSec: number,
  ...
  for (const ep of getDataselectCandidates(activeId)) {
    const u =
      ep.url +
      '?net=' +
      encodeURIComponent(station.net) +
      '&sta=' +
      encodeURIComponent(station.sta) +
      '&loc=' +
      encodeURIComponent(loc) +
      '&cha=' +
      encodeURIComponent(cha) +
      '&starttime=' +
      encodeURIComponent(si) +
      '&endtime=' +
      encodeURIComponent(ei) +
      '&format=miniseed';
    try {
...
async function discoverStream(activeId: DataCenterId, station: Station): Promise<StreamChoice | null> {
  ...
  const urls = [
    ...
    c.stationUrl +
      '?level=channel&format=text&network=' +
      encodeURIComponent(station.net) +
      '&station=' +
      encodeURIComponent(station.sta) +
      '&starttime=' +
...
async function probeStation(
  ...
  for (const ep of getDataselectCandidates(activeId)) {
    ...
    try {
      ...
      if (tokenRef.current !== ownToken) return 'pending';
      if (!r.ok) {
        return r.status === 404 || r.status === 204 ? 'offline' : 'partial';
      }
      const b = await r.arrayBuffer();
      if (tokenRef.current !== ownToken) return 'pending';
...
export default function LiveStationsPanel() {
  ...
  const [listMessage, setListMessage] = useState<{ kind: 'loading' | 'error' | 'empty'; text: string } | null>({
    kind: 'loading',
  ...
  const [pos, setPos] = useState<{ x: number; y: number }>(() => ({
    x: Math.max(24, (typeof window !== 'undefined' ? window.innerWidth : 1200) - 900 - 24),
    y: 80,
  }));
  ...
  const setStatus = useCallback((code: string, status: StationStatus) => {
    setStatusMap((prev) => {
      const next = new Map(prev);
      next.set(code, status);
      return next;
    });
  }, []);

  const runSweep = useCallback(
    async (list: Station[], token: symbol) => {
      ...
      for (let i = 0; i < list.length; i += STATUS_BATCH) {
        ...
        const results = await Promise.all(
          batch.map((s) => probeStation(activeId, s, sweepTokenRef, token)),
        );
        if (sweepTokenRef.current !== token) return;
        setStatusMap((prev) => {
          const next = new Map(prev);
          batch.forEach((s, idx) => {
            if (results[idx] !== 'pending') next.set(s.code, results[idx]);
          });
          return next;
        });
  ...
  const loadStations = useCallback(
    async (activeId: DataCenterId, net: string) => {
      setListMessage({ kind: 'loading', text: 'Loading stations…' });
      setStations([]);
      ...
      setSelected(null);
      setStream(null);
      setViewerStatus('Click a station to view live waveform');

      const token = Symbol('sweep');
      ...
      if (!text) {
        setListMessage({ kind: 'error', text: `Could not load stations (HTTP ${lastStatus})` });
        return;
      }
      ...
      if (parsed.length === 0) {
        setListMessage({ kind: 'empty', text: `No stations returned for network ${net}` });
        return;
      }

      setStations(parsed);
      setListMessage(null);
      runSweep(parsed, token);
    },
  ...
  useEffect(() => {
    ...
    const c = DATA_CENTERS[centerId];
    const nets = Object.keys(c.networks);
    if (!nets.includes(networkCode)) {
      setNetworkCode(nets[0]);
      return;
    }
  ...
  useEffect(() => {
    return () => {
      clearLiveTimer();
      sweepTokenRef.current = Symbol('unmount');
    };
  }, [clearLiveTimer]);
  ...
  const fetchAndDraw = useCallback(async () => {
    ...
    try {
      ...
      if (!buf || buf.byteLength === 0) {
        setViewerStatus('No MiniSEED data currently available');
        return;
      }
      const parsed = parseMiniSEED(new Uint8Array(buf), 50000);
      if (parsed.length === 0) {
        setViewerStatus('No samples in MiniSEED data');
        return;
      }
      const step = Math.ceil(parsed.length / 5000);
      const ds = parsed.filter((_, i) => i % step === 0);
      const c = canvasRef.current;
      if (c) drawWaveform(c, ds);
      setHasWaveform(true);
      setViewerStatus(
        `${s.code} · ${st.cha}/${st.loc} · ${parsed.length.toLocaleString()} pts`,
      );
    } catch {
      setViewerStatus('Waveform fetch failed');
    }
  }, []);
  ...
  const openStation = useCallback(
    async (station: Station) => {
      ...
      clearLiveTimer();
      clearWaveformCanvas();
      setViewerStatus(`Discovering channels for ${station.code}…`);

      const found = await discoverStream(centerIdRef.current, station);
      if (!found) {
        setViewerStatus('No live channels available');
        return;
      }
      setStream(found);
      streamRef.current = found;
      setViewerStatus(`Live · ${found.cha}/${found.loc}…`);
      await fetchAndDraw();
      clearLiveTimer();
      liveTimerRef.current = window.setInterval(fetchAndDraw, 10000);
    },
  ...
  useEffect(() => {
    ...
    fetchAndDraw();
  }, [windowSec, selected, stream, fetchAndDraw]);
  ...
  const onDragMove = (e: React.PointerEvent) => {
    ...
    if (!d) return;
    const w = panelRef.current?.offsetWidth ?? 900;
    const h = panelRef.current?.offsetHeight ?? 560;
    const x = Math.max(0, Math.min(window.innerWidth - w, e.clientX - d.dx));
    const y = Math.max(0, Math.min(window.innerHeight - h, e.clientY - d.dy));
    setPos({ x, y });
  };
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label="Live stations monitoring"
    >
      <div
        className="stations-header"
      ...
      >
        <div className="stations-title">
          ...
          <span>Live Stations Monitoring</span>
        </div>
        <button
          type="button"
          className="stations-close"
          aria-label="Close stations panel"
          onClick={() => setStationsOpen(false)}
      ...
      <div className="stations-body">
        <aside className="stations-sidebar">
          <div className="stations-controls">
            <select
              value={centerId}
              onChange={(e) => setCenterId(e.target.value as DataCenterId)}
            >
              {centerList().map((c) => (
            ...
            <select value={networkCode} onChange={(e) => setNetworkCode(e.target.value)}>
              {networks.map(([code, label]) => (
          ...
          <div className="stations-stats">
            {listMessage
              ...
              : filtered.length === stations.length
                ? `${stations.length} stations · ${center.label} · ${networkCode}`
                : `${filtered.length} / ${stations.length} stations · ${networkCode}`}
          </div>
          <div className="stations-list custom-scrollbar">
            {listMessage && listMessage.kind !== 'empty' && (
              <div className={`stations-msg ${listMessage.kind}`}>{listMessage.text}</div>
            )}
            {!listMessage && filtered.length === 0 && (
              <div className="stations-msg loading">No stations match</div>
            )}
            {filtered.map((s) => {
              const status = statusMap.get(s.code) || 'pending';
              const isActive = selected?.code === s.code;
              return (
                <button
                  key={s.code}
                  type="button"
                  className={`station-item${isActive ? ' is-on' : ''}`}
                  onClick={() => openStation(s)}
                >
                  <div className="station-row">
                    <span className={`sdot ${status}`} title={dotTitle(status)} />
        ...
        <section className="stations-viewer">
          <div className="viewer-header">
            <span className="viewer-title">
              {selected ? `${selected.code} · ${selected.name}` : 'Waveform Viewer'}
            </span>
            <button
              type="button"
          ...
          <div className="viewer-controls">
            ...
            <div className="viewer-window-btns">
              {WINDOW_OPTIONS.map((opt) => (
                <button
                  key={opt.seconds}
                  type="button"
                  className={`window-btn${windowSec === opt.seconds ? ' is-on' : ''}`}
                  onClick={() => setWindowSec(opt.seconds)}
                >
                  {opt.label}
                </button>
              ))}
          ...
          <div className="viewer-chart">
            {!hasWaveform && (
              <div className="viewer-placeholder">
                {selected ? 'Waiting for MiniSEED data…' : 'Select a station'}
                <span>
                  {selected
                    ? `${selected.code} — fetching live samples`
                    : 'Click any station in the list to load its live MiniSEED waveform'}
                </span>
              </div>
            )}
            <canvas
              ref={canvasRef}
              style={{ display: hasWaveform ? 'block' : 'none' }}
            />
          </div>
          {selected && (
            <div className="viewer-info">
              <div className="info-item">
                ...
                <div className="info-value">{selected.sta}</div>
              </div>
              <div className="info-item">
                <div className="info-label">Network</div>
                <div className="info-value">{selected.net}</div>
              </div>
              <div className="info-item">
                <div className="info-label">Latitude</div>
                <div className="info-value">{selected.lat.toFixed(4)}°</div>
              </div>
              <div className="info-item">
                <div className="info-label">Longitude</div>
                <div className="info-value">{selected.lon.toFixed(4)}°</div>
              </div>
              <div className="info-item">
                <div className="info-label">Channel</div>
                <div className="info-value">{stream ? `${stream.cha} / ${stream.loc}` : '—'}</div>
              </div>
              <div className="info-item">
                <div className="info-label">Data Center</div>
                <div className="info-value">{center.label}</div>
              </div>
            </div>
          )}
        </section>
