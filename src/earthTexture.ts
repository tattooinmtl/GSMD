...
async function loadTile(z: number, x: number, y: number): Promise<HTMLImageElement | null> {
  try {
    return await loadImage(`/api/osm/${z}/${x}/${y}.png`);
  } catch {
    return null;
  }
}
...
/** Stitch OSM (via local proxy) into an equirectangular texture matching latLngToCartesian. */
export async function loadOsmEarthTexture(zoom = 4): Promise<THREE.Texture> {
  const n = 2 ** zoom;
  const tileSize = 256;
  const world = n * tileSize;
  const merc = document.createElement('canvas');
  merc.width = world;
  merc.height = world;
  const mctx = merc.getContext('2d')!;
  mctx.fillStyle = '#aad3df';
  mctx.fillRect(0, 0, world, world);

  const jobs: { x: number; y: number }[] = [];
  for (let y = 0; y < n; y++) {
    for (let x = 0; x < n; x++) jobs.push({ x, y });
  }

  let painted = 0;
  await mapPool(jobs, 6, async ({ x, y }) => {
    const img = await loadTile(zoom, x, y);
    if (img) {
      mctx.drawImage(img, x * tileSize, y * tileSize, tileSize, tileSize);
      painted += 1;
    }
  });
  ...
  for (let y = 0; y < eqH; y++) {
    ...
    const y1 = Math.min(world - 1, y0 + 1);
    const fy = my - y0;
    for (let x = 0; x < eqW; x++) {
      const mx = (x / eqW) * world;
      const x0 = Math.floor(mx) % world;
      const x1 = (x0 + 1) % world;
      const fx = mx - Math.floor(mx);
      const i00 = (y0 * world + x0) * 4;
      const i10 = (y0 * world + x1) * 4;
      const i01 = (y1 * world + x0) * 4;
      const i11 = (y1 * world + x1) * 4;
      const o = (y * eqW + x) * 4;
      for (let c = 0; c < 3; c++) {
        const v0 = src.data[i00 + c] * (1 - fx) + src.data[i10 + c] * fx;
        const v1 = src.data[i01 + c] * (1 - fx) + src.data[i11 + c] * fx;
        dst.data[o + c] = v0 * (1 - fy) + v1 * fy;
      }
      dst.data[o + 3] = 255;
    }
  }
...
export async function loadGlobeBaseTexture(): Promise<THREE.Texture> {
  try {
    return await loadOsmEarthTexture(3);
  } catch {
    return loadNasaEarthTexture();
  }
}

export async function loadGlobeDetailTexture(): Promise<THREE.Texture | null> {
  try {
    return await loadOsmEarthTexture(4);
  } catch {
    return null;
  }
}
