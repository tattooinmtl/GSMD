...
export const useStore = create<AppState>((set) => ({
  ...
  setApiKey: (key) => { localStorage.setItem('nasa_api_key', key); set({ apiKey: key }); },
  showApiKeyModal: false,
  setShowApiKeyModal: (show) => set({ showApiKeyModal: show }),
  layers: defaultLayers,
  toggleLayer: (id) => set((state) => ({ layers: state.layers.map((l) => l.id === id ? { ...l, enabled: !l.enabled } : l) })),
  setLayerOpacity: (id, opacity) => set((state) => ({ layers: state.layers.map((l) => l.id === id ? { ...l, opacity } : l) })),
  events: [],
  setEvents: (events) => set({ events }),
  docks: defaultDocks,
  toggleDock: (id) => set((state) => ({ docks: state.docks.map((d) => d.id === id ? { ...d, isOpen: !d.isOpen } : d) })),
  ...
  selectEvent: (event, opts) => {
    if (!event) { set({ selectedEvent: null }); return; }
    const pair = event.geometry?.[0] ? extractLonLat(event.geometry[0].coordinates) : null;
    if (opts?.flyTo && pair) {
      set({ selectedEvent: event, focusTarget: { lng: pair[0], lat: pair[1], token: Date.now() } });
      return;
    }
  ...
  setTheme: (theme) => { localStorage.setItem('earth_theme', theme); applyDocumentTheme(theme); set({ theme }); },
  minimaxKey: typeof window !== 'undefined' ? localStorage.getItem('minimax_api_key') || '' : '',
  setMinimaxKey: (key) => { localStorage.setItem('minimax_api_key', key); set({ minimaxKey: key }); },
  nvidiaKey: typeof window !== 'undefined' ? localStorage.getItem('nvidia_api_key') || '' : '',
  setNvidiaKey: (key) => { localStorage.setItem('nvidia_api_key', key); set({ nvidiaKey: key }); },
  aiProvider: (() => {
