...
function chatOutputText(data) {
  ...
  if (Array.isArray(msg?.content)) {
    const text = msg.content.map((c) => c.text || c.content || "").filter(Boolean).join("\n").trim();
    if (text) return text;
  }
  // MiniMax M1 (reasoning) may return content in reasoning_content when content is empty
...
async function openaiChat({ url, apiKey, model, messages, extra }) {
  ...
  try {
    ...
    // non-JSON error page from upstream
  }
  if (!response.ok) {
    const msg = data.error?.message || data.message || data.base_resp?.status_msg
      || (raw ? raw.slice(0, 240) : `LLM ${response.status}`);
    throw new Error(msg);
  }
  return data;
}
        ...
        try {
          if (req.method === "POST" && url === "/api/ai/chat") {
            ...
            if (provider === "nvidia") {
              ...
              if (!apiKey) {
                sendJson(res, 400, {
                  error: "Missing NVIDIA key. Add NVIDIA_API_KEY to .env or paste it in Providers.",
                });
                return;
              }
              ...
              sendJson(res, 200, { text: chatOutputText(data), id: data.id, citations: [] });
              return;
            }

            if (provider !== "minimax") {
              sendJson(res, 400, { error: `Unknown provider "${provider}". Use minimax or nvidia.` });
              return;
            }
            ...
            if (!apiKey) {
              sendJson(res, 400, {
                error: "Missing MiniMax key. Add MINIMAX_API_KEY to .env or paste it in Providers.",
              });
              return;
            }
          ...
          if (req.method === "POST" && url === "/api/ai/fetch") {
            ...
            const upstream = await fetch(target, {
              headers: { "User-Agent": "EarthMonitorLibrarian/1.0" },
              redirect: "follow",
            });
            ...
            if (ctype.includes("application/json")) {
              ...
              sendJson(res, 200, {
                ...
                text: JSON.stringify(json).slice(0, 12000),
              });
              return;
            }
